...
- Repo: shhgit secret scanner (Go module github.com/trebor048/shhgot) at C:\Users\me\GIT\shhgit2\shhgit; default mode is terminal-only, with web and tui modes and 5 logFormat presets (minimal, fancy, ultra fancy, even more fancy, neon). go.mod `go 1.26.0` matches install.sh/install.ps1 FALLBACK_GO_VERSION and Dockerfile GO_VERSION=1.26. Full file-by-file sweep complete (all sources, tests, scripts, workflows, dashboard, Dockerfile, LICENSE reviewed); no bugs outstanding.
- Layout: the binary is `package main` under `cmd/shhgit/` (the dashboard is embedded from `cmd/shhgit/dashboard/index.html`); shared libraries are `core/`, `aiproviders/`, `reviewstore/`; a second tool binary lives at `cmd/apikey-check/` (not in the Docker image, which builds only ./cmd/shhgit; its `found` counter double-counts but is never read, and `envLike` is an identity function — both harmless, left untouched).
- Concurrent sibling agents edit this working tree; transient build failures (e.g., cmd/shhgit/tui.go) resolve on their own — verify before assuming your change broke the build. Background swarm subagents have been unreliable (return no reports); the task/loop agent path works — prefer it for delegated analysis.
- Verification commands for this repo: go build -o shhgit-test.exe ./cmd/shhgit, go vet ./..., go test -run 'TestLog|TestSecret|TestLink|TestFile|TestLocal|TestEntropy|TestSignature' -v ./cmd/shhgit, go test ./.... Tests run with the package directory as CWD, so a test that needs a repo-root file must resolve it from runtime.Caller, not a bare relative path. The suite has exactly one legitimate t.Skip (aiproviders/provider_test.go:1482, Windows mode bits) and no TODO/FIXME markers.
...
- Full verification gate: gofmt + build + vet + go test ./... (390 passed in 5 packages) + `$env:CGO_ENABLED='1'; go test -race -count=1 ./core/ ./cmd/shhgit/ ./aiproviders/` (348 passed) + deadcode (only 6 known pre-existing unreachable funcs, left by design) + a --local smoke scan against a scratch dir in $env:TEMP (expect exit 1 + a [SECRET] line). Smoke recipe: AWS docs example keys are blacklisted by design and classic ghp_ tokens are NOT matched — use a synthetic `github_pat_` + 80+ alphanumerics.
